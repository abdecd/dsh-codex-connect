...
const OPENAI_CODEX_GPT6_PICKER_MODELS = [{
	...OPENAI_CODEX_GPT6_MODEL,
	id: "gpt-6-luna",
	name: "GPT-6 Luna"
}, {
	...OPENAI_CODEX_GPT6_MODEL,
...
*/
function createOpenAICodexAdapter(credentials, resolveAttachments, fastMode, resolveEnabledModels) {
	...
	const provider = {
		...
		getModels: () => {
			...
			const additions = [OPENAI_CODEX_GPT6_MODEL, ...OPENAI_CODEX_GPT6_PICKER_MODELS].filter((model) => !models.some((existing) => existing.id === model.id));
	...
	adapter.listModels = async (providerId) => {
		...
		if (enabledModels === void 0) return listed;
...
const SUPPORTED_DSH_PLUGIN_API_VERSION = "0.1.5-rc.2";
...
const DEFAULT_OPENAI_CODEX_ENABLED_MODELS = OPENAI_CODEX_MODEL_OPTIONS.map((model) => model.id);
const REPLACED_OPENAI_CODEX_MODEL_IDS = /* @__PURE__ */ new Map([["gpt-5.6-luna", "gpt-6-luna"], ["gpt-5.6-sol", "gpt-6-solar"]]);
function normalizeModelId(id) {
	return REPLACED_OPENAI_CODEX_MODEL_IDS.get(id) ?? id;
}
function normalizeEnabledModels(modelIds) {
...
function resolveOpenAICodexSettings(value) {
	return {
		...
		...value,
		enabledModels: value.enabledModels === void 0 ? [...DEFAULT_OPENAI_CODEX_ENABLED_MODELS] : normalizeEnabledModels(value.enabledModels)
	};
}
...
function decodeOpenAICodexSettings(value) {
	...
	if (enabledModels !== void 0 && (!Array.isArray(enabledModels) || enabledModels.length === 0 || enabledModels.some((model) => typeof model !== "string" || model.trim().length === 0))) return void 0;
	...
	return {
		enabledModels: enabledModels === void 0 ? [...DEFAULT_OPENAI_CODEX_ENABLED_MODELS] : normalizeEnabledModels(enabledModels),
		enableSearch,
		enableImageTool,
...
const OPENAI_CODEX_SETTINGS_NS = brandString(OPENAI_CODEX_SETTINGS_NAMESPACE);
...
function apply(ctx, config) {
	...
	ctx.llm.registerAdapter([OPENAI_CODEX_PROVIDER], createOpenAICodexAdapter(credentials, () => ctx.get("attachments"), fastMode, () => resolveOpenAICodexSettings(current()).enabledModels));
	if (!ctx.llm.listConfigurableProviders().some((entry) => entry.provider === "openai-codex")) ctx.llm.registerConfigurableProviders([{
		provider: OPENAI_CODEX_PROVIDER,
		displayName: "OpenAI Codex",
	...
	ctx.inject(["webServer", "connection"], (webCtx) => registerOpenAICodexAuthRoutes(webCtx, credentials, void 0, fastMode));
	let stopped = false;
	let searchFiber;
...
//#endregion
export { OPENAI_CODEX_FAST_MODE_PATH as $, OPENAI_CODEX_TRANSPORT_API_VERSION as A, openAICodexConflictMessage as B, resolveOpenAICodexSettings as C, OPENAI_CODEX_IMAGE_MAX_RESPONSE_BYTES as D, OPENAI_CODEX_IMAGE_MAX_ERROR_BYTES as E, isOpenAICodexTransportError as F, DSH_PLUGIN_API_PACKAGES as G, COMPATIBILITY_CONTRACT as H, VIEW_IMAGE_TOOL_NAME as I, SUPPORTED_NODE_RANGE as J, PI_AI_PACKAGE as K, IMAGE_GENERATE_TOOL_NAME as L, OPENAI_CODEX_TRANSPORT_SERVICE as M, OpenAICodexTransport as N, OPENAI_CODEX_IMAGE_PROMPT_MAX_LENGTH as O, OpenAICodexTransportError as P, evaluateCompatibility as Q, assertNoOpenAICodexProviderConflict as R, decodeOpenAICodexSettings as S, OPENAI_CODEX_IMAGE_MAX_COUNT as T, COMPATIBILITY_PACKAGES as U, CODEX_CONNECT_VERSION as V, COMPATIBILITY_SCHEMA_VERSION as W, assessCompatibility as X, SUPPORTED_PI_AI_VERSION as Y, detectCompatibility as Z, DEFAULT_OPENAI_CODEX_SEARCH_MODE as _, name as a, normalizeTrustedOrigin as at, OPENAI_CODEX_MODEL_OPTIONS as b, migrateOpenAICodexSearchHistory as c, readOpenAICodexRateLimits as ct, OPENAI_CODEX_SEARCH_URL as d, openAICodexAuthStatus as dt, FastModeRegistry as et, OpenAICodexSearchProvider as f, OPENAI_CODEX_AUTH_FILENAME as ft, DEFAULT_OPENAI_CODEX_SEARCH_MAX_OUTPUT_TOKENS as g, DEFAULT_OPENAI_CODEX_SEARCH_CONTEXT_SIZE as h, openAICodexAuthPath as ht, inject as i, OpenAICodexTrustedOriginsStore as it, OPENAI_CODEX_TRANSPORT_ERROR_CODES as j, OPENAI_CODEX_IMAGE_REQUEST_TIMEOUT_MS as k, OPENAI_CODEX_BASE_URL as l, loginOpenAICodex as lt, DEFAULT_OPENAI_CODEX_ENABLED_MODELS as m, OpenAICodexCredentialStore as mt, OPENAI_CODEX_SETTINGS_NS as n, OPENAI_CODEX_FAST_MODE_MAX_SESSION_ID_LENGTH as nt, OPENAI_CODEX_HISTORY_BACKUP_SUFFIX as o, OPENAI_CODEX_USAGE_URL as ot, mapOpenAICodexSearchResponse as p, OPENAI_CODEX_PROVIDER as pt, SUPPORTED_DSH_PLUGIN_API_VERSION as q, apply as r, isFastModeSessionId as rt, OPENAI_CODEX_SEARCH_MODEL_REQUEST_EVENT as s, parseOpenAICodexUsage as st, Config as t, OPENAI_CODEX_FAST_MODE_MAX_SESSIONS as tt, OPENAI_CODEX_SEARCH_PROVIDER as u, logoutOpenAICodex as ut, DEFAULT_OPENAI_CODEX_SEARCH_MODEL as v, OPENAI_CODEX_IMAGE_GENERATION_URL as w, OPENAI_CODEX_SETTINGS_NAMESPACE as x, DEFAULT_OPENAI_CODEX_SETTINGS as y, diagnoseOpenAICodex as z };
