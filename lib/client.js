window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const modelSelectionStyle = {
			display: "grid",
			gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))",
			gap: 8
		};
		const modelOptionStyle = {
			display: "flex",
			alignItems: "flex-start",
			gap: 9,
			padding: "8px 10px",
		...
		function sameField(left, right, field) {
			return field === "enabledModels" ? sameStringList(left.enabledModels, right.enabledModels) : left[field] === right[field];
		}
		function sameConfig(left, right) {
			return left !== void 0 && right !== void 0 && CONFIG_FIELDS.every((field) => sameField(left, right, field));
		}
		...
		function OpenAICodexConfiguration({ scope, t }) {
			...
			const validModels = draft !== void 0 && draft.enabledModels.length > 0;
			const valid = validModel && validTokens && validModels;
			const toggleModel = (modelId) => {
				if (draft === void 0) return;
				const enabledModels = draft.enabledModels.includes(modelId) ? draft.enabledModels.filter((id) => id !== modelId) : [...draft.enabledModels, modelId];
			...
			const save = async () => {
				...
				try {
					for (const field of CONFIG_FIELDS) {
						...
						if (accepted !== void 0 && sameField(accepted, desired, field)) continue;
						const value = field === "enabledModels" ? [...desired.enabledModels] : desired[field];
						await scope.set(field, value);
						const next = scope.getSnapshot().value;
						if (next === void 0 || !sameField(next, desired, field)) throw new Error(`Host refused ${field}`);
					}
					const accepted = scope.getSnapshot().value;
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
				...
				children: [
						...
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h4", {
									id: "openai-codex-model-selection-title",
									style: labelStyle,
									children: t("modelSelectionHeading")
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
									style: {
										...bodyStyle$1,
									...
									children: t("modelSelectionIntro")
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									...
									children: OPENAI_CODEX_MODEL_OPTIONS.map((model) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
					...
					!validModels && draft !== void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
						style: errorStyle$1,
						role: "alert",
						children: t("invalidEnabledModels")
					}) : null,
		...
		*/
		...
		const en = {
			...
			modelSelectionHeading: "Models shown in the picker",
			modelSelectionIntro: "Choose which Codex models appear in the conversation model picker. This only filters discovery; it does not change the current or default route.",
			invalidEnabledModels: "Select at least one Codex model.",
		...
		const zh = {
			...
			modelSelectionHeading: "模型选择器显示的模型",
			modelSelectionIntro: "选择对话模型选择器中显示哪些 Codex 模型。这里只过滤模型目录，不会改变当前或默认路由。",
			invalidEnabledModels: "至少选择一个 Codex 模型。",
		...
		function GeneratedImages({ images, load, t }) {
			const [urls, setUrls] = (0, react.useState)(/* @__PURE__ */ new Map());
			(0, react.useEffect)(() => {
				let active = true;
				Promise.all(images.map(async (image) => [image.attachmentId, await load(image)])).then((entries) => {
					if (active) setUrls(new Map(entries));
				}).catch(() => {
					if (active) setUrls(/* @__PURE__ */ new Map());
				});
				return () => {
					active = false;
				};
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				"data-testid": "codex-generated-images",
				style: {
					display: "grid",
					gap: 8
				},
				children: images.map((image, index) => {
					const url = urls.get(image.attachmentId);
					const label = image.name ?? `${t("image")} ${String(index + 1)}`;
					return url === void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						style: detail,
						children: t("loading")
					}, image.attachmentId) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("img", {
						src: url,
						alt: label,
						style: {
							display: "block",
		...
		function CodexImageToolView({ block, sessionId, t, sessions }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ResponsiveCard, {
				...
				}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(GeneratedImages, {
				...
				side: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("details", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("summary", {
						...
						children: decoded.images.map((image, index) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: t("imageDetail", {
							name: image.name ?? String(index + 1),
							format: formatMediaType(image.mediaType),
							width: image.width,
							height: image.height,
